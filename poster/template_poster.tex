...
\[
f(x) - f(y) \leq  \nabla f(y)^T(x - y) + \frac{L}{2}\|x -y\|^2\qquad (x, y \in \dom f)
\]
\medskip
\hrule\medskip
...
\begin{itemize}
\item step size satisifies $0 < \eta < 1/\|A\|_\mathrm{op}$
...
\item \emph{Disclaimer:} this statement (nor its proof)
  ...
  quite slowly (although, this is not what we observe in practice, \dots)
\end{itemize}
\medskip
\hrule\medskip
...
\item Consider a subsequential limit (one exists since
  ...
  \end{itemize}
\item The analysis above applies to any limit, so we're done (here we use
...
\item {\color{gray}Show that once the boundary is reached, successive iterates remain on the
  ...
  $t \geq \tau^{\mathrm{bd}}$.
\item Show a contraction inequality of the form
  \[
  \|x^{(k + 1)} - x^\star\| \leq (1 - \epsilon) \|x^{(k)} - x^{\star}\|
  \qquad (\epsilon > 0)
  \]
  once $k \geq \tau^{\mathrm{bd}}$.
...
Thanks to Yair and John for putting up with our stupid questions.
