...
\Title{Projected Gradient Descent Efficiently Solves the Trust Region Problem}
\end{textblock}
...
\end{array}
\end{equation}
with variable $x$. The matrix $A$ may be indefinite, in which case the resulting problem is not convex.

\medskip
\hrule\medskip
\Head{Projected Gradient Descent}\\
We investigate the behavior of \textbf{projected gradient descent} (PGD)
which begins at an initialization $x^{(0)} \in \R^n$ and generates iterates
\begin{equation}
\begin{array}{ll}
    y^{(k + 1)} &= x^{(k)} - \eta \nabla f(x^{(k)})  \\
