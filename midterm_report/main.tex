...
we are interested in procedures that provably return a ($\epsilon$-suboptimal) global minimizer $x \in \R^n$.
...
\begin{lem}[PGD is a descent method]
...
\begin{proof}
  ...
  strengthing the third condition to $(u_1^T \xopt)(u_1^T b) < 0$. But this implies that
...
\begin{figure}
    ...
    \caption{Three random indefinite instances of the the trust region subproblem \eqref{problem:TR}, with $\eta = 1/(2\|A\|_{\mathrm{op}})$ and
