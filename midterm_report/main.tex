...
\end{lem}
\begin{proof}
As $x^{(k)} \in \B(R)$, the $\beta$-smoothness of $f$ and Lemma \ref{lem:variational-char} imply
\[
f(x^{(k + 1)}) - f(x^{(k)}) 
\leq 
...
\left(\frac{\beta}{2} - \frac{1}{2\eta}\right)  \|x^{(k + 1)} - x^{(k)}\|^2,
\]
as needed. The second inequality follows from noting that plugging $x^{(k)}$ into the
